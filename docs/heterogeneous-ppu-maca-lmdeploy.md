...
```bash
docker pull crpi-z3i6df2ze96lh4ld.cn-hangzhou.personal.cr.aliyuncs.com/deeplink_infer/deeplink_infer:maca-lmdeploy
```
...
docker run -itd \
   ...
   --entrypoint /bin/bash \
   "crpi-z3i6df2ze96lh4ld.cn-hangzhou.personal.cr.aliyuncs.com/deeplink_infer/deeplink_infer:maca-lmdeploy"
```
...
```bash
docker pull crpi-z3i6df2ze96lh4ld.cn-hangzhou.personal.cr.aliyuncs.com/deeplink_infer/deeplink_infer:v1.5.2-lmdeploy0.10.0
```
...
docker run --privileged=true \
  ...
  -v "${HOST_MODEL_DIR}:${MODEL_PATH}" \
  -v /var/run/docker.sock:/var/run/docker.sock \
  crpi-z3i6df2ze96lh4ld.cn-hangzhou.personal.cr.aliyuncs.com/deeplink_infer/deeplink_infer:v1.5.2-lmdeploy0.10.0
```
